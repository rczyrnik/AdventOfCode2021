        ...
        if path[-1] == "end":
            ...
            for choice in map_dict[path[-1]]:
                if choice.lower() == choice and choice in path:
                    lower_dict = Counter(path)
                    previously_visited_twice = False
                    for lower_case in lower_set:
                        if lower_dict[lower_case] > 1:
                            previously_visited_twice = True
                    if not previously_visited_twice:
...
lower_set = {key for key in map_dict.keys() if key.lower() == key}
lower_set.remove('start')
# print(lower_set)
