...
def print_board(board):
    print(''.join((str(x) for x in board[0:5])))
    print(''.join((str(x) for x in board[5:10])))
    print(''.join((str(x) for x in board[10:15])))
    print(''.join((str(x) for x in board[15:20])))
    print(''.join((str(x) for x in board[20:25])))
...
def play_game():
    ...
    for number in numbers:
        ...
        for board, scoreboard in zip(boards, scoreboards):
            if number in board: 
                scoreboard[board.index(number)] = 1
        for board, scoreboard in zip(boards, scoreboards):
            if check_board(scoreboard, winners): 
                scoreboards.remove(scoreboard)
                boards.remove(board)

        # if there are no boards left, return the most recent board
        if len(boards) == 0:
            return(board, scoreboard, number)
